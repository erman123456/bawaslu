...
@extends('layouts.app',
            [
                'checkPoint' => 'sengketa',
                'pageTitle'=>"sengketa$data[id_pilih]",
                'mainPage'=>'Data Sengketa',
            ]
        )
@section('content')
@php
        ...
        <div class="adv-table editable-table ">
          ...
          @endif
              <div class="btn-group pull-right">
               <button type="button" class="btn btn-primary btn-xs tambahsengketa" data-toggle="modal" data-target="#form_modal">
                  Tambah Data
               </button>
              </div>
           </div>
                         ...
                         <tbody>
                             ...
                             <tr>
                                 ...
                                 <td  class='text-center'><?php  if($row->putusan_penyelesaian) { ?> 
                                    <a href="{{route('sengketa.putusan_penyelesaian_sengketa', $row->uid)}}" class="btn btn-label-right"><i class="fa fa-download"> Download</i></a> <?php } else{ echo "Data Tidak Tersedia"; } ?> </td>

                                 <td  class='text-center'><?php  if($row->tindak_lanjut) { ?>
                                    <a href="{{route('sengketa.tindak_lanjut_sengketa', $row->uid)}}" class="btn btn-label-right"><i class="fa fa-download"> Download</i> </a> <?php } else{ echo "Data Tidak Tersedia"; } ?> </td>
                                    
                                 
                                 <td>
                                    <a href="#" class="btn btn-warning btn-xs float-right ml-1 Editsengketa" data-toggle="modal" data-target="#form_modal" data-id="{{$row->uid}}" title="Edit"><i class="fa fa-edit"></i></a>

                                     <button type="button" rel="tooltip" class="btn btn-xs btn-danger btnHapus-sengketa" data-original-title="Hapus" title="Hapus" id="{{$row->uid}}"><i class="fa fa-trash-o"></i></button>
                                 </td>
                             </tr>
                             @endforeach
                         </tbody>
         ...
         <div class="modal-body">
           ...
           @endif
            ...
            <input type="hidden" id="id_pilih" name="id_pilih" value="{{$id_pilih}}">
               <input type="hidden" id="url" name="url" value="{{url("sengketa_edit")}}" class="form-control" >
               <div class="row"> 
                <div class="col-lg-4">
                    <section class="panel">
                        <div class="panel-body">
                    ...
                    <section class="panel">
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                <small id="l_PutusanPenyelesaian" class="form-text text-muted">If Your File doesn't Exists, Please Insert Your file</small>
                            </div>
                        </div>
                    </section>
                </div>
...
//  Tambah Paslon
$("body").on("click", ".tambahsengketa" , function(){
   $('#judulModal').html('Tambah Data sengketa');
   $('.modal-footer button[type=submit]').html('Save');
   $('.modal-body form').attr('action', "{{ route ('sengketa_input')}}");
...
$('.Editsengketa').on('click', function(){
   $('#judulModal').html('Edit Data sengketa');
   $('.modal-footer button[type=submit]').html('Edit');
   $('.modal-body form').attr('action', "{{ route ('sengketa_update')}}")
...
$("body").on("click" , ".btnHapus-sengketa", function(){
	var id = this.id;
	...
	swal({
		...
		swal({
			...
			document.location.href = "{{ url ('sengketa_delete')}}?uid="+id + "&id_pilih=" + id_pilih;
		});
		} 
		else {
